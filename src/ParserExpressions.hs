...
pExpressionList = do
        expressionList <-
                pExpression `sepby0` (ParseNode <$> sat (== Symbol ','))
        return ParseTree { what = "expressionList", children = expressionList }

pExpression :: Parser ParseResult
pExpression = do
        terms <- pTerm `sepby1` pOp
        return ParseTree { what = "expression", children = terms }

pTerm :: Parser ParseResult
pTerm =
        (   (wrap . ParseNode)
                <$> ((sat isIntegerConstant) +++ (sat isStringConstant))
                )
    ...
    where
        wrap child = ParseTree { what = "term", children = [child] }
        wrapChildren children =
                ParseTree { what = "term", children = children }

pKeywordConstant :: Parser ParseResult
pKeywordConstant = ParseNode <$> sat
        (\x ->
                x
                        == Keyword "true"
                        || x
